...
export interface AutoUICollectionProps<T> {
	...
	/** Pass a map component to display the position of each item instead of a table */
	mapRenderer?: AutoUIContext<T>['mapRenderer'];
	...
	onRowClick?: AutoUIContext<T>['onRowClick'];
}
...
export const AutoUICollection = <T extends AutoUIBaseResource<T>>({
	...
	const autouiContext = React.useMemo(() => {
		const latitudeField = getFieldForFormat(model.schema, 'latitude');
		const longitudeField = getFieldForFormat(model.schema, 'longitude');
